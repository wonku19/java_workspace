 ...
 * 
 * 상속의 장점
 * - 적은 양의 코드로 새로운 클래스 작성이 가능하다 (자식 클래스들에서 공통적인 부분을 부모 클래스로 빼버리기 때문에)
 * - 코드를 공통적으로 관리하기 때문에 코드의 추가나 변경이 용이하다
 * - 코드에 중복을 제거하여 프로그램의 생산성과 유지 보수에 크게 기여한다
 * 
 * 상속의 특징
 * - 클래스 간의 상속은 단일 상속만 가능하다 (부모클래스를 하나만 가져올 수 있다)
 * - 명시되어 있지 않아도 모든 클래스는 Object 클래스를 상속
 *   -> Object 클래스에서 제공하는 메서드 들을 오버라이딩 하며 메서드 재정의가 가능
 *   -> 이 클래스로 예를 들면 public class Notebook 뒤에 보이지 않는 extends Object 가 있다는 뜻임
 * - 부모 클래스의 생성자, 초기화 블록은 상속되지 않는다.
 *   -> 자식 클래스 생성 시에 부모 클래스 생성자가 먼저 실행
 * - 부모의 private 멤버는 상속이 되지만 직접 접근이 불가능
 *   -> Getter / Setter 사용해서 간접적으로 접근해야 한다
 */

public class Notebook extends Product { // extends Product 후 import 필수
...
//	private int price; // 부모클래스에 기재했기 때문에 삭제해도됨
	...
	public Notebook(String brand, String pCode, String name, int price, String cpu) {
		 ...
		 *  super.pCode = pCode; // 부모가 default - 같은 패키지에 있어야지만 직접 접근이 가능함
		 *  super.name = name; // 부모가 protected - 상속관계일 때 자식 객체에서 직접 접근이 가능함
		 *  super.price = price; // 부모가 public - 어디서든 접근이 가능
		 * 2. 부모의 Setter 메서드를 호출해서 초기화
		 *  - 부모 클래스에서 setter 메서드를 제공하면 지식 객체에서 부모의 setter 메서드를 통해서 필드의 초기화가 가능
		 ...
		 * 3. 부모 생성자 호출해서 초기화
		 */
		super(brand, pCode, name, price); // 앞에 1, 2 여러가지 방법이 있지만 3번 방법에 제일 편함		
		this.cpu = cpu;
	}
