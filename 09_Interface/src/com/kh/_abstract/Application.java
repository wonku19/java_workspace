 ...
 * - 미완성 클래스이기 떄문에 new연산자를 통해서 객체를 생성할 수 없다.
 ...
 * - 미완성 메서드로 {}(중괄호) 가 구현되지 않음
 * - 추상 클래스를 상속받는 자식 클래스에서 반드시 오버라이딩(재정의) 해주지 않으면 컴파일 에러가 발생된다.
...
//		Sports sport = new Sports(); // Sports 가 추상클래스이기 때문에 객채생성이 안됨
