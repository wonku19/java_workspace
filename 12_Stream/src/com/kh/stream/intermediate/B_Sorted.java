...
//		b.method1();
	...
	public void method1() {
		...
		Arrays.stream(new int[] {2, 1, 3, 5, 4}).boxed().sorted(Comparator.reverseOrder()).forEach(value -> System.out.print(value + " "));
		// 
	}
	...
	public void method2() {
		...
		// 오름차순 정렬 = Comparable 인터페이스 구현 내용에 따라서 정렬 = .sorted()
		names.stream().sorted().forEach(name -> System.out.print(name + " "));
		
		System.out.println();
		
		// 내림차순 정렬 = Comparable 인터페이스 구현 내용에 반대로 정렬 = .sorted(Comparator.reverseOrder())
		names.stream().sorted(Comparator.reverseOrder()).forEach(name -> System.out.print(name + " "));
		
		System.out.println();
		
		// 만들어둔 객체(Student) 나이순 정렬
