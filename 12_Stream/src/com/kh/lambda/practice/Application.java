 ...
 * - 자바에서 함수적 프로그래밍을 위해서 자바 8부터 람다식을 지원함
 * - 람다식은 매개변수를 가지는 함수와 같은 코드 블록이지만 런타임 시에는 인터페이스의 익명 구현 객체를 생성
 * - 람다식을 사용하면 코드가 간결해지고 컬렉션의 요소들을 필터링 하거나 매핑해서 원하는 결과를 쉽게 가져올 수 있음
 * - 쉽게 말해서 함수 이름이 없는 익명 함수라고 보면됨 / 자바스크립트에서 화살표 함수와 유사함
 */
...
public class Application {
	...
	public static void main(String[] args) {
		...
		MathInterface minusLambda = (int x, int y) -> x - y; // 한줄의 함수는 중괄호({}), 리턴(return) 을 안써도 됨
